...
function getForecast(latitude, longitude){
    ...
    fetch(api)
        ...
        .then(function(data){
            for(let i = 0 ; i < 8 ; i ++){ // 8 시간대의 정보를 담음
...
function addForecast() {
    ...
    for (let i = 0; i < 8; i++) { //3시간씩 8번, 24시간  
        const li = document.createElement("li");
        const template = document.querySelector("#forecastTemp");
        const clone = document.importNode(template.content, true);

        clone.querySelector(".day").textContent = `${forecast[i].day} ${forecast[i].time}시`;
        clone.querySelector(".img").src = `icons/${forecast[i].iconId}.png`;
        clone.querySelector(".forecastTemperature").textContent = `${forecast[i].temperature}도씨`;
